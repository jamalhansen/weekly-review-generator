...
class TestSummarizeCLI:
    @patch("weekly_review.cli.find_vault_root")
    @patch("weekly_review.cli.resolve_provider")
    @patch("weekly_review.cli.write_review_section")
    def test_summarize_json_output(self, mock_write, mock_resolve, mock_find_vault):
        ...
        with patch("weekly_review.cli.load_daily_notes_for_week") as mock_load:
